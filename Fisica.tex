...
%   Titolo Meglio (Appunti di Fisica, AA 2022/2023, etc); (nah)
% V Introduzione SI
...
dove la posizione cresce quadraticamente rispetto al tempo. 
...
\begin{gather*}
    \displaystyle x(t)=C_1e^{\left(-\frac{\gamma}{2}+ i\Omega\right)t}+C_2e^{\left(-\frac{\gamma}{2}- i\Omega\right)t}\\
...
\begin{gather*}
    ...
    \Im\left(e^{-\frac{\gamma}{2}t}(C_1\cos(\Omega t)+iC_1\sin(\Omega t)+C_2\cos(\Omega t)-iC_2\sin(\Omega t))\right)=0\\
    \begin{cases}
...
\\
La componente ortogonale all'asse di rotazione si esprime tramite:
\begin{equation*}
...
Poiché questi cicli sono reversibili si avrà per ognuno dei cicli:
\begin{gather*}
    \displaystyle\frac{Q_i}{|Q_j|}=\frac{T_i}{T_j}\\
    \displaystyle\frac{Q_i}{T_i}-\frac{|Q_j|}{T_j}=0
\end{gather*}
Clausius dimostrò che la somma per ogni sorente di queste componenti è minore o uguale a $0$, in base al tipo di trasformazione: 
\begin{quotation}
...
\begin{gather*}
    \Gamma_{{rev}}:\Delta S_{AB}=\displaystyle\int_{\Gamma_{AB}}\frac{\delta Q}{T}=\frac{1}{T}\int\delta Q=\frac{\Delta Q_{AB}}{T}\Rightarrow T\Delta S_{AB}=\Delta Q_{AB}\\
