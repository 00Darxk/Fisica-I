...
% ? \cancelto{value}{expression} nelle equazioni dove può essere utile;
...
\begin{center}\begin{tikzpicture}[scale=2]
    %in caso mi servano griglie le copio da qua
...
\begin{center}\begin{tikzpicture}[scale=2]
    ...
    \draw[->, thick] (O) -- (1, 0) coordinate (v) node[below] {$\vec{v}$};
    \draw[->, thick] (O) -- (0.5, 0.5) coordinate (w) node[above] {$\vec{w}$};
    ...
    \draw[->, thick] (O) -- (0, 1) node[left] {$\vec{v}\times\vec{w}$};
    \draw[->, thick] (O) -- (0,-1) node[left] {$\vec{w}\times\vec{v}$};
\end{tikzpicture}\end{center}
...
\begin{gather}
    ...
    \rho = v_{xy}  = vsin\theta\\
    v_z = vcos\theta\\
    \vec{v} = vsin\theta cos\varphi\hat{x} + vsin\theta sin\varphi\hat{y} + vcos\varphi\hat{z}
\end{gather}
...
viene chiamata spostamento: $\vec{s}(t) = \Delta\vec{r}(t)$, quando 
...
\begin{gather}
    ...
    a(t)=\dot v(t) = \ddot x(t) = -\omega^{2}Asin(\varphi+\omega t) = -\omega^{2}x(t)
\end{gather}
...
\begin{equation}
    \ddot x(t) = -\omega x(t) \Rightarrow
    d^{2}x(t) = -\omega x(t)dt^{2} \Rightarrow
    \displaystyle\iint d^{2}x(t)= -\omega\iint x(t)dt^{2} \Rightarrow
...
\begin{center}\begin{tikzpicture}[scale =2]
        ...
        \draw[->] (0,1.2)--(0.5,1.7) node[above]{$\vec{v}_0$};
        \draw[->] (0,1.2)--(0,1.7) node[left]{$\vec{v}_{y,0}$};
        \draw[->] (0,1.2)--(0.5,1.2) node[below]{$\vec{v}_{x,0}$};
...
\begin{center}\begin{tikzpicture}[scale=2]
    ...
    \draw[->, thick](r)--(r1);
    \node[right] at (0.42,0.9){$d\vec{r}(t)$};
...
\begin{equation}
    ...
    r\omega^{2}(-sin\theta(t)\hat{y}-cos\theta(t)\hat{x})
\end{equation}

Si definisce il versore $\hat{\nu}(t)=-(cos\theta(t)\hat{x}+sin\theta(t)\hat{y})=-\hat{r}(t)\implies$

\begin{equation}
    \vec{a}(t)=r\omega^{2}\hat{\nu}(t)=-\omega^{2}r\cdot\hat{r}(t)=-\omega^{2}\vec{r}(t)\land a=\omega^{2}r=\displaystyle\frac{v^{2}}{r}
\end{equation}
...
\begin{center}\begin{tikzpicture}[scale=2]
    ...
    \draw[->](1.5,0)--(1.25,0.25)node[right]{$\tau(t)$};
...
\begin{equation}
    \vec{\omega}(t)=\omega(t)\cdot\hat{\beta}\land \vec{\alpha}(t)=\alpha(t)\cdot\hat{\beta}
\end{equation}
...
\begin{gather}
    \begin{cases}
        ...
        r\alpha(t)\hat{\beta}\times(-\hat{\nu}(t))+r\omega(t)\hat{\beta}\times\hat{\tau}(t)=
...
di moti circolare in ogni intorno della posizione all'istante $t$, 
...
%accellerazione centripeta costante. La risultante sarà 
...
\begin{center}
    \begin{tikzpicture}[scale=2]
        ...
        \draw[->,very thick](-1,0)coordinate(O)--(2,0)node[right]{$\vec{r}$};
        \draw[->,very thick](O)--(1,0.866)node[left]{$\vec{r}+d\vec{r}$};
...
\begin{center}
    \begin{tikzpicture}[scale=2]
        ...
        \node[above]at(0.6,0){$U^{eff}(r)(S_{Soele})$};
    \end{tikzpicture}
\end{center}
...
\begin{equation}
    \sum\vec{F}_y=\vec{0}\Rightarrow mg\hat{y}-b(v_{lim}-v_0)\hat{y}=0\hat{y}\Rightarrow v_{lim}=\tau g +v_0
\end{equation}
\begin{center}\begin{tikzpicture}[scale=2]
    \node[below]at(0.2,0){$O$};
...
\begin{center}\begin{tikzpicture}[scale=2]
    ...
    \node[left]at(0,1){$x_{in}e^{-\displaystyle\frac{\gamma}{2}t}$};
    \node[left]at(0,-1){$-x_{in}e^{-\displaystyle\frac{\gamma}{2}t}$};
    \node[above]at(3,0.2){$x_{in}e^{-\displaystyle\frac{\gamma}{2}t}cos(\Omega t)$};
...
\begin{center}\begin{tikzpicture}[scale=2]
    \draw[->](-0.5,0)--(2.5,0)node[above]{$\omega_f$};
    \draw[->](0,-0.25)--(0,2.5)node[right]{$A(\omega_f)$};

    \draw[-]plot[smooth,domain=-0.5:0.94](\x,{0.22+0.045*e^(1/(1.2-\x))});
    \draw[-]plot[smooth,domain=1.06:2.5](\x,{0.22+0.045*e^(1/(\x-0.8))});
    \draw[-]plot[smooth,domain=-0.5:2.5](\x,{0.21+1.4*e^-(\x-1)^2});
    \draw[-]plot[smooth,domain=-0.5:2.5](\x,{0.2+0.8*e^-(\x-1)^2});
    \draw[dashed](1,0)node[below]{$\omega_M$}--(1,2.5);
\end{tikzpicture}\end{center}

\subsection{Moti Relativi e Forze Apparenti}
Un corpo in un sistema di riferimento sarà descritto da 
epsressioni rispetto a quel sistema, considerandone altri cambierà 
l'espressione in coordinate del comportamento del corpo 
interno al sistema.
...
\begin{center}\begin{tikzpicture}[scale=2]
    ...
    \draw[->,thick](0,-1)--(0,0)node[left]{$\vec{R}$};
...
\begin{center}\begin{tikzpicture}[scale=2]
    \draw[->](0,0)--(2,0)node[above]{$p$};
    \draw[->](0,0)--(0,2)node[right]{$V$};
    \draw[->](0,0)--(-1,-1)node[left]{$T$};    
    \node[above]at(0.5,0.5){$A$};
    \node[below]at(0.2,-0.5){$B$};
    \draw[->]plot[smooth]coordinates{(0.5,0.5)(0.55,0.1)(0.4,-0.2)};
    \draw[-]plot[smooth]coordinates{(0.4,-0.2)(0.3,-0.3)(0.2,-0.5)};
    \path[->,draw,snake it]
        (0.5,0.5)--(0.1,0)--(0.2,-0.5);
\end{tikzpicture}\end{center}
...
simielad una trasformazione isoterma, ma l'adiabatica presente 
una pendenza sempre maggiore.

\begin{center}\begin{tikzpicture}[scale=2]
    \draw[->](0,-0.5)--(0,2)node[above]{$p$};
    \draw[->](-0.5,0)--(2,0)node[above]{$V$}; 
    \draw[->,ultra thick]plot[smooth,domain=0.55:1](\x,{-0.2+0.7/(\x)^2});   
    \draw[-,ultra thick]plot[smooth,domain=1:1.7](\x,{-0.2+0.7/(\x)^2});
    \draw[-,ultra thick]plot[smooth,domain=0.55:1](\x,{0.5+0.7/(\x)^2});   
    \draw[<-,ultra thick]plot[smooth,domain=1:1.7](\x,{0.5+0.7/(\x)^2});
\end{tikzpicture}\end{center}
...
\begin{center}\begin{tikzpicture}[scale=2]
    ...
    \draw[->,snake it,thick](0.5,0.5)--(1,0.5);
    \draw[-,snake it,thick](1,0.5)--(1.3,0.576)node[below]{$B$};
\end{tikzpicture}\end{center}
...
Un sistema se trasforma tutto il calore assorbito in lavoro 
allora cambierà il suo stato, non potrà quindi ripetere 
la stessa trasformazione, sarà necessario un'altra fonte di 
lavoro per permettere il sistema a ritornare allo stato iniziale. 
Vengono quindi considerati solamente cicli termodinamici.  
Questo è uno dei motivi per cui non può esistere un moto 
perpetuo. \\ 
Per trasformare il calore in lavoro è necessaria una trasformazione 
isoterma. Per ritornare allo stato iniziale si possono considerare 
due trasformazioni isocope, ed un'altra isoterma, ad una temperatura $T_1$ 
minore della prima. 
...
Comportandosi come una macchina di Kelvin. Quindi se non può esistere una 
...
poiché è una trasformazione irreversibile la sua efficienza sarà minore dell'efficienza di una macchina di Carnot, per cui l'integrale di Clausius su tutta la trasformazione 
sarà minore di $0$:
